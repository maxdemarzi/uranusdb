...
public class FastUtilGraph implements Graph {
    ...
    private Object2IntMap<String> getOrCreateNodeKey(String label) {
    ...
    public boolean deleteNodeProperties(int id) {
        ...
        current.put("~id", id);
        current.put("~label", current.get("~label"));
        current.put("~key", current.get("~key"));
    ...
    public boolean removeRelationship(int id) {
        ...
        int node1 = (int)relationship.get("~incoming_node_id");
        int node2 = (int)relationship.get("~outgoing_node_id");
        int count = (int)relationship.getOrDefault("~count", 1);
    ...
    public boolean removeRelationship(String type, String label1, String from, String label2, String to, int number) {
        ...
        if (count == 1) {
            ...
            if (count != number) {
                ...
                if (number > 1) {
                    relationships.get(movedRelId).put("~count", number);
                } else {
    ...
    public Iterator getAllNodeIdsDoesNotWork() {
        IntIterator[] intIterators = new IntIterator[nodeKeys.keySet().size()];
        int i = 0;
        for (String s : nodeKeys.keySet()) {
            intIterators[i] = nodeKeys.get(s).values().iterator();
    ...
    public Iterator getAllRelationshipIds() {
